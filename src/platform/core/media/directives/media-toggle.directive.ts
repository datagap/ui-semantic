...
@Directive({
  selector: '[uiMediaToggle]',
})
export class UiMediaToggleDirective implements OnInit, OnDestroy {
  ...
  set query(query: string) {
    if (!query) {
      throw new Error('Query needed for [uiMediaToggle] directive.');
    }
    this._query = query;
  }
  ...
  constructor(private _renderer: Renderer2, private _elementRef: ElementRef, private _mediaService: UiMediaService) {}
