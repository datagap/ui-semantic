...
@Directive({
  selector: '[uiFullScreen]',
  exportAs: 'uiFullScreen',
})
export class UiFullscreenDirective {
  ...
  private _getFullScreenElement(): Element {
    const { _document }: UiFullscreenDirective = this;
    const uiFullScreenElementMap: object = {
    ...
    for (const props of Object.keys(uiFullScreenElementMap)) {
      if (_document[props]) {
